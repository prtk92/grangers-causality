...
oosmean = pd.DataFrame(columns=['Machine_ID','Scratchers_Sales','Out_of_Stock__','Lagged_Correlation','ssr_ftest','ssr_chi2test','lrtest','params_ftest'])
for i in machineid :
    ...
    if((pd.isna(oos_i["Scratchers_Sales"].corr(oos_i["Out_of_Stock__"]))) or (oos_i.shape[0] < 30) or oos_i["Out_of_Stock__"].std() < 0.00000001):
        ...
        gc_res = grangercausalitytests(oos_i, [1])
        gc_row = pd.Series([i,
